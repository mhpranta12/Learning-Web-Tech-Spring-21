    ...
    if(isset($_POST['submit'])){
     if(empty($_POST['name']))                        // Name null checking  . . . . . .
        ...
        if ($f==0)                                                      // making sure that is everything okay or not ...
        {
            ...
            file_get_contents('../Model/assistant_info.json');

            if (file_exists('../Model/assistant_info.json'))
            {
                $get_data= file_get_contents('../Model/assistant_info.json'); 
                $array_data =json_decode($get_data);
                $uinfo = array('name'=>$username ,
                'password'=>$password,
                'fullname' => $name , 
                'email'=>$email,
                'degree' => $degree ,
                ...
                if(file_put_contents('../Model/assistant_info.json',$json_data))
                {
